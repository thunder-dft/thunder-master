...
! begin iammaster
              ...
              write (filename,                                               &
     &               '("/rho_ontopL_", i2.2,".",i2.2,".",i2.2,".dat")')&
...
!    &              status = 'unknown', position = 'append')
     &              status = 'unknown', position = 'append')
              write (13,100) pFdata_bundle%nFdata_cell_2c, P_rho_ontopL,     &
     &                       isorp, filename(2:30), pFdata_cell%nME, ndd_rho,&
     &                       dmax
              close (unit = 13)
...
! begin iammaster
              ...
              write (filename,                                               &
     &               '("/rho_ontopR_", i2.2,".",i2.2,".",i2.2,".dat")')      &
     &          isorp, species(ispecies)%nZ, species(jspecies)%nZ
              inquire (file = trim(Fdata_location)//trim(filename), exist = skip)
              if (skip) cycle
              open (unit = 11, file = trim(Fdata_location)//trim(filename),  &
     ...
     &              file = trim(Fdata_location)//trim(interactions),         &
     &              status = 'unknown', position = 'append')
              write (13,100) pFdata_bundle%nFdata_cell_2c, P_rho_ontopR,     &
     &                       isorp, filename(2:30), pFdata_cell%nME, ndd_rho,&
     &                       dmax
              close (unit = 13)
