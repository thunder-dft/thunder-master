...
! Input
        ...
        real*8, intent (in) :: rhozz
        real*8, intent (in) :: rhopz
...
! ===========================================================================
	...
	real*8 dummy
        ...
        real*8 dzup, dzdn, dzzup, dzzdn, dpzup, dpzdn
...
! Input variab required by easypbe subroutine
	...
	uplap =  dppup + dpup/r + dzzup                    !4.d0*zup*dup*(zup-1.d0/r)
	dnlap = dppdn + dpdn/r + dzzdn                         !4.d0*zdn*ddn*(zdn-1.d0/r)
	
        d = dup+ddn
        agrad = sqrt(rhop**2 + rhoz**2)
	delgrad = rhop*(rhop*rhopp + rhoz*rhopz)/agrad + rhoz*(rhop*rhopz + rhoz*rhozz)/agrad                  !4.d0*agrad*(zup**2*dup+zdn**2*ddn)
		
	agrup = sqrt(dpup**2 + dzup**2)
	agrdn = sqrt(dpdn**2 + dzup**2)
	
	delgrup = dpup*(dpup*dppup + dzup*dpzup)/agrup + dzup*(dpup*dpzup + dzup*dzzup)/agrup
	delgrdn = dpdn*(dpdn*dppdn + dzdn*dpzdn)/agrdn + dzdn*(dpdn*dpzdn + dzdn*dzzdn)/agrdn
...
!         This is probably where I have to make changes
      t=agr/(twoksg*rho)
