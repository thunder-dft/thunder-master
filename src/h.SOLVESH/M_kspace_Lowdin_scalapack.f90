...
! M_kspace_Lowdin_elpa.f90
! Module Description
! ===========================================================================
!      This is a version of kspace.f90 that uses the ELPA and ScaLAPACK library.
...
! ===========================================================================
        ...
        ! for MPI ierr
        integer ierr, iscf_iteration_slave
...
! Initialize logfile
        logfile = s%logfile
        !every rank print the nkpoints value
        ...
        if (my_proc .eq. 0) then
          ! broadcast the iscf_iteration from rank 0 to all other ranks, and check the return value ierr
          call MPI_Bcast (iscf_iteration, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
...
! DIAGONALIZE THE OVERLAP MATRIX
          ...
          do while (.TRUE.)
            ! broadcast the iscf_iteration from rank 0 to all other ranks
            call MPI_Bcast (iscf_iteration_slave, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
            if(iscf_iteration_slave .eq. -1) then
              call finalize_mpi
              return
            end if
            call diagonalization_initialize_slave (s, iscf_iteration_slave, ikpoint=1)

! DIAGONALIZE THE OVERLAP MATRIX - SLAVE
            if (iscf_iteration_slave .eq. 1) call diagonalize_S_slave (s)
            call diagonalize_H_Lowdin_slave(s, iscf_iteration_slave, ikpoint=1)
          end do
        end if
