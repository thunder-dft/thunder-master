...
! Initialize logfile
        ...
        ! write (logfile,*) ' Time to assemble ewald interactions: ', time2-time1
        ! stop "stop here, finish ewald"
! Initialize the charge transfer bit
        do iatom = 1, s%natoms
          in1 = s%atom(iatom)%imass
          Q0(iatom) = 0.0d0
...
! The charge transfer bit = need a +-dq on each orbital.
        ...
        ! kokkos
        real(rk), pointer, dimension(:,:)  :: c_ratom, c_g123, c_Qall, c_ewald, c_atom_ewald
        real(rk), pointer, dimension(:,:,:) :: c_dewald
        type(gamma_view2d) :: v_c_ratom, v_c_g123, v_c_Qall, v_c_ewald, v_c_atom_ewald
        type(gamma_view3d) :: v_c_dewald
        integer, dimension(3) :: igALLmax, c_il123mx
        real(rk), pointer, dimension(:,:) :: c_aALLvec
        type(gamma_view2d) ::  v_c_a123vec
        logical :: USEKOKKOS
...
! ===========================================================================
  ! allocate dualviews
        call kokkos_allocate_dualview( c_ratom, v_c_ratom%view, 'c_ratom', int(s%natoms, c_size_t), int(3, c_size_t) )
        call kokkos_allocate_dualview( c_g123, v_c_g123%view, 'c_g123', int(3, c_size_t), int(3, c_size_t) ) !( x/y/z, g1/g2/g3)
        call kokkos_allocate_dualview( c_g123, v_c_g123%view, 'c_g123', int(3, c_size_t), int(3, c_size_t) ) !( x/y/z, g1/g2/g3)
        call kokkos_allocate_dualview( c_Qall, v_c_Qall%view, 'c_Qall', int(s%natoms, c_size_t), int(2, c_size_t) ) !(natoms, Q0/Q)
        call kokkos_allocate_dualview( c_dewald, v_c_dewald%view, 'c_dewald', int(3, c_size_t), int(s%natoms, c_size_t), int(s%natoms, c_size_t) ) !(x/y/z, iatom, jatom)
        call kokkos_allocate_dualview( c_ewald, v_c_ewald%view, 'c_ewald', int(s%natoms, c_size_t), int(s%natoms, c_size_t) )
        call kokkos_allocate_dualview( c_atom_ewald, v_c_atom_ewald%view, 'c_atom_ewald', int(s%natoms, c_size_t), int(3, c_size_t) ) !(natoms, xyz)
        call kokkos_allocate_dualview( c_aALLvec, v_c_a123vec%view, 'c_a123vec', int(3, c_size_t), int(3, c_size_t) ) !( a1/2/3vec, x/y/z)
        ! copy data from s%atom(iatom)%ratom(:) to c_ratom(:,:)
        do iatom = 1, s%natoms
          c_ratom(iatom,1) = s%atom(iatom)%ratom(1)
          c_ratom(iatom,2) = s%atom(iatom)%ratom(2)
          c_ratom(iatom,3) = s%atom(iatom)%ratom(3)
        end do
        ! write(*,*) ' Time to allocate kokkos dualviews and copy data to kokkos dualviews: ', time2-time1
        !!!!!!!!!!!!test ratom !!!!!!!!!!!!
        ! call print_ratom(v_c_ratom)
        ! print all ratom into a file f_ratom.txt, with each line has three columns
        ! open(unit=10, file='f_ratom.txt', status='unknown', action='write')
        ! do iatom = 1, s%natoms
        !   write(10,'(F10.6, 1X, F10.6, 1X, F10.6)') s%atom(iatom)%ratom(1), s%atom(iatom)%ratom(2), s%atom(iatom)%ratom(3)
        ! end do
        !!!!!!!!!!!!finish test ratom !!!!!!!!!
  ! Calculate nuclear charge.
        do iatom = 1, s%natoms
          ...
          c_Qall(iatom,1) = Q0(iatom)
          c_Qall(iatom,2) = Q(iatom)
        end do
! Initialize ewald to zero and initialize other quantities
        a1vec = s%lattice(1)%a
        a2vec = s%lattice(2)%a
        a3vec = s%lattice(3)%a
! Initialize view c_aALLvec for kokkos
        c_aALLvec(1, :) = a1vec
        c_aALLvec(2, :) = a2vec
        c_aALLvec(3, :) = a3vec
...
! Now we do the same thing for r2 and r3
        ...
        call gamma1(ig1mx, ig2mx, ig3mx, kappa,  v_c_ratom, v_c_ewald, v_c_dewald, v_c_atom_ewald, v_c_g123, v_c_Qall, s%volume, s%natoms, s%icluster, v_c_a123vec, c_il123mx)
        !********** Whether use Kokkos *************!
        UseKokkos = .true.
        if (UseKokkos) then
          ! if(kind(c_ewald) == kind(1.0_real64)) then
          !   write(*,*) "use double precision and kokkos for gamma"
          !   s%ewald => c_ewald
          !   s%dewald => c_dewald 
          !   nullify(c_ewald)
          !   nullify(c_dewald)
          ! else
            ! write(*,*) "use single precision and kokkos for gamma"
          ...
          ! endif
          do iatom = 1, s%natoms
            s%forces(iatom)%ewald = c_atom_ewald(iatom, :)
          end do
        else ! Do not use Kokkos result, use old fortran code to compare result
...
!                   qq = (Q(iatom) - Q0(iatom))*(Q(jatom) - Q0(jatom))
     ...
     &                               + erfc(argument))/z**3
        ...
        ! write (s%logfile,*) ' Time to compute gamma2: ', time2-time1
...
! There are no forces.
      ...
      endif ! whether use KOKKOS

        !!!!!!!!!!!!!!!!!!!!!!!!print out fortran result !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ! open(unit=111, file='f_ewald.txt', status='unknown', action='write')
        ! open(unit=112, file='f_dewald.txt', status='unknown', action='write')
        ! open(unit=113, file='f_atom_ewald.txt', status='unknown', action='write')
        ! do iatom = 1, s%natoms
        !   write(113,'(F15.13, 1X, F15.13, 1X, F15.13)') s%forces(iatom)%ewald(1), s%forces(iatom)%ewald(2), s%forces(iatom)%ewald(3)
        !   do jatom = 1, s%natoms
        !     write(112,'(F15.13, 1X, F15.13, 1X, F15.13)') s%dewald(1,iatom,jatom), s%dewald(2,iatom,jatom), s%dewald(3,iatom,jatom)
        !     write(111,'(F15.13)') s%ewald(iatom,jatom)
        !   end do
        ! end do
        !!!!!!!!!!!!!!!!!!!!!!!!Finish print out fortran result !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
